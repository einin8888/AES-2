...
//復号で使用する関数
void invaddroundkey(int m[4][4],int k[11][4][4],int count){
    //printf("**********\naddroundkey\n");
    //printf("No.%d-----\n",count);
    for(int i = 0;i < 4;i++){
        for(int j = 0;j < 4;j++){
            //printf("m[%d][%d] = m[%d][%d] ^ subkey[%d][%d][%d]\n",j,i,j,i,count,i,j);
            //printf("m[%d][%d] = %3x ,k[%d][%d][%d] = %3x \n",j,i,m[j][i],count,i,j,k[count][i][j]);
            m[j][i] = m[j][i] ^ k[count][i][j];
            //printf("%x \n",m[j][i]);
        }
        //printf("\n");
    }
